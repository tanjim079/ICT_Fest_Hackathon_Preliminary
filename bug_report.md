...
This document presents a comprehensive diagnostic and resolution report for the **21 architectural, logical, and concurrency bugs** discovered within the CoWork API application. 
...
### **BUG-21: Rate Limiter Concurrency (Multi-Process Bypass)**
* **Target Location**: `app/services/ratelimit.py` (Line 20, inside `record_and_check()`)
* **Symptom / Business Impact**: The rate limiter utilized local, in-memory python locks and dictionaries to track request timestamps. Under a multi-worker server environment, requests handled by different worker processes completely bypassed the synchronization limits, allowing users to exceed the rolling 60-second limit.
* **Faulty Logic (Before)**:
  ```python
  ...
  ```
* **Applied Fix (After)**:
  Migrated rate limiting to utilize a database-backed log table (`rate_limit_logs`) and wrapped query and insert operations inside a SQLite `BEGIN IMMEDIATE` write lock transaction:
  ```python
  db.execute(text("BEGIN IMMEDIATE"))
